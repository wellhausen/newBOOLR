...
toolbar.message = function (msg, type) {
  ...
  if (type == "warning") {
    ...
    toast.innerHTML +=
      "<button onclick='undo()' style='font-family: Ubuntu'><span class='material-icons'>undo</span>Undo</button>";
  }
...
for (let i = 0; i < listItems.length; ++i) {
  listItems[i].onmouseenter = function () {
    this.style.background = "#222";
  };
  listItems[i].onmouseleave = function () {
    this.style.background = "#111";
  };
  listItems[i].onmouseup = function () {
    this.onclick();
  };
}

...
(function () {
  function createInput(component, property, value, valid, errormsg, apply) {
  ...
  function createTextArea(component, property, value, valid, errormsg, apply) {
  ...
  function createSelect(component, property, value, options, apply) {
  ...
  dialog.editComponent = function (component) {
    ...
    dialog.container.innerHTML +=
      "<p class='material-icons' style='font-size: 60px; margin: 10px'>edit<p>";
    ...
    const width = createInput(
      ...
      (width) =>
        width > 0 &&
        2 * (+width + component.height) >=
          component.input.length + component.output.length,
    ...
    if (component.properties.hasOwnProperty("delay")) {
      inputs.push(
        createInput(
          ...
          function () {
            ...
            createVariableReference(this.value, component, [
              "properties",
              "delay",
            ]);
          }
      ...
      dialog.container.removeChild(
        dialog.container.children[dialog.container.children.length - 1]
      );
    ...
    if (component.properties.hasOwnProperty("frequency")) {
      inputs.push(
        createInput(
          component.properties,
          "frequency",
          component.properties.frequency,
      ...
      inputs.push(
        createTextArea(
          ...
          function () {
            ...
            let data = Array(
              Math.pow(2, component.properties.addressWidth)
            ).fill(0);
  ...
  dialog.editPort = function (port) {
    ...
    const pos = createInput(
      ...
      port.pos.pos,
      (pos) =>
        side.valid(side.value) &&
        +pos >= 0 &&
        +pos <
          (+side.value % 2 == 0
            ? port.component.width
            : port.component.height) &&
        !findPortByComponent(port.component, +side.value, +pos),
  ...
  dialog.editDelay = function (component, callback) {
    ...
    dialog.container.innerHTML +=
      "<i class='material-icons' style='font-size: 60px'>access_time<i>";
  ...
  dialog.editRom = function (component, callback) {
    ...
    dialog.container.innerHTML +=
      "<i class='material-icons' style='font-size: 60px'>memory<i>";
    ...
    const addressWidthInput = createInput(
      component.properties,
      "addressWidth",
      component.properties.addressWidth || "4",
    ...
    const dataWidthInput = createSelect(
      component.properties,
      "dataWidth",
      component.properties.dataWidth || 4,
      [
        { value: 4, text: "4" },
        { value: 8, text: "8" },
        { value: 16, text: "16" },
    ...
    const dataInput = createTextArea(
      ...
      function () {
        ...
        const contents = this.value.replace(/\s/g, "").toUpperCase();
        const dataWidth = component.properties.dataWidth;
        let data = Array(Math.pow(2, component.properties.addressWidth)).fill(
          0
        );
        for (let i = 0; i < data.length; i++) {
          const start = (i * dataWidth) / 4;
    ...
    dialog.addOption("OK", function () {
      if (
        addressWidthInput.valid(addressWidthInput.value) &&

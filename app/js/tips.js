...
setInterval(function () {
  ...
  if (connecting) {
    if (
      (!tips.connecting_ctrl.disabled && mouse.screen.x < 100) ||
      mouse.screen.x > c.width - 100 ||
      mouse.screen.y < 100 ||
      mouse.screen.y > c.height - 100
    ) {
      ...
      if (
        x.join("") == x[0].toString().repeat(8) ||
        y.join("") == y[0].toString().repeat(8)
      ) {
  ...
  if (!tips.waypoints.disabled && Math.random() < 0.01) {
    tips.waypoints.show();
  }
}, 500);

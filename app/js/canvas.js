...
function isVisible(x, y, w, h) {
  return (
    x + (w || 0) > offset.x &&
    x < offset.x + c.width / zoom &&
    y - (h || 0) < offset.y &&
    y > offset.y - c.height / zoom
  );
}
...
const changeZoom = (dz) => {
  zoomAnimation = Math.min(Math.max(zoomAnimation + dz, 2), 300);
};
  ...
  if (hoverBalloon.display) {
    if (Array.isArray(mouse.hover.pos)) {
      hoverBalloon.style.top = `${
        mouse.screen.y - hoverBalloon.clientHeight - 35
      }px`;
      hoverBalloon.style.left = `${
        mouse.screen.x - hoverBalloon.clientWidth / 2
      }px`;
    } else {
      hoverBalloon.style.top = `${
        (offset.y - mouse.hover.pos.y - 0.5) * zoom -
        hoverBalloon.clientHeight -
        35
      }px`;
      hoverBalloon.style.left = `${
        (mouse.hover.pos.x - offset.x + mouse.hover.width / 2 - 0.5) * zoom -
...
c.onmouseenter = (e) => {
  e.which > 0 && (scrollAnimation.animate = false);
};
    ...
    if (e.shiftKey) {
      if (selecting) {
        ...
        (function animate() {
          ...
          if (
            Math.abs(x - selecting.x - selecting.w) * zoom > 1 ||
...
c.onmousemove = function (e) {
  ...
  if (e.which == 1) {
    if (selecting && !selecting.components) {
      ...
      if (e.shiftKey) {
        if (!connecting.hasOwnProperty("lock")) {
          if (e.movementX != e.movementY)
            connecting.lock = Math.abs(e.movementX) < Math.abs(e.movementY);
        } else {
      ...
      scrollAnimation.v =
        Math.sqrt(Math.pow(e.movementX, 2) + Math.pow(e.movementY, 2)) / zoom;

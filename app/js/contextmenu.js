...
contextMenu.show = function (pos) {
  ...
  if (selecting) {
    ...
    if (selecting.components && selecting.components.length > 2)
      this.appendChild(contextOptions["componentize"]);
    this.appendChild(contextOptions["remove all"]);
  } else {
    const component = findComponentByPos(
      Math.round(pos.x / zoom + offset.x),
      Math.round(-pos.y / zoom + offset.y)
    );
    if (component) {
      if (component.constructor == Wire) {
        ...
        component.hasOwnProperty("name") &&
          this.appendChild(contextOptions["edit name"]);
        component.hasOwnProperty("delay") &&
          this.appendChild(contextOptions["edit delay"]);
      ...
      contextOptions[
        "set waypoint"
      ].innerHTML = `<i class="material-icons">my_location</i><span>Set waypoint @${component.name} (S)</span>`;
      ...
      contextOptions[
        "set waypoint"
      ].innerHTML = `<i class="material-icons">my_location</i><span>Set waypoint @${Math.round(
        contextMenu.pos.x
      )},${Math.round(contextMenu.pos.y)} (S)</span>`;

      this.appendChild(contextOptions["goto waypoint"]);
      contextOptions["goto waypoint"].innerHTML =
        '<i class="material-icons">redo</i><span>Jump to waypoint (W)</span>';
      if (waypoints.length == 0)
        contextOptions["goto waypoint"].className = "disabled";
      else {
        contextOptions["goto waypoint"].className = "";
        contextOptions["goto waypoint"].innerHTML +=
          '<i class="material-icons" style="float: right; margin: 0">navigate_next</i>';
      }
    }
  }
  ...
  if (pos.x > c.width - this.clientWidth)
    this.pos.x = (c.width - this.clientWidth) / zoom + offset.x;
  if (pos.y > c.height - this.clientHeight)
    this.pos.y = -(c.height - this.clientHeight) / zoom + offset.y;
};
...
contextOptions["edit name"].innerHTML =
  '<i class="material-icons">mode_edit</i><span>Edit name (E)</span>';
  ...
  if (component && component.hasOwnProperty("name")) {
    popup.prompt.show(
      ...
      (name) =>
        name &&
        name.length < 18 &&
        action("edit", [component, "name", (n) => name], true)
    );
  }
};
...
contextOptions["edit color"].innerHTML =
  '<i class="material-icons">color_lens</i><span>Edit color (E)</span>';
  ...
  if (component && component.color_off) {
    popup.color_picker.show(
      (color) =>
        ...
        !edit(component, "color_off", color) &&
        action("edit", [component, "color_on", lighter(color, 50)], true)
    );
  }
};
...
contextOptions["edit delay"].innerHTML =
  '<i class="material-icons">timer</i><span>Edit delay</span>';
contextOptions["edit delay"].onclick = () => {
  ...
  if (component && component.hasOwnProperty("delay")) {
    popup.prompt.show("Edit delay", "Enter the new delay in ms", (delay) => {
...
contextOptions["rotate"].innerHTML =
  '<i class="material-icons">rotate_left</i><span>Rotate (R)</span>';
...
contextOptions["copy"].innerHTML =
  '<i class="material-icons">content_copy</i><span>Copy to clipbord (Ctrl+C)</span>';
...
contextOptions["paste"].innerHTML =
  '<i class="material-icons">content_paste</i><span>Paste (Ctrl+V)</span>';
...
contextOptions["remove component"].innerHTML =
  '<i class="material-icons">delete</i><span>Remove (Del)</span>';
contextOptions["remove component"].onclick = () => {
  if (findComponentByPos()) {
    action("remove", findComponentByPos(), true);
  }
};

// Remove wire
contextOptions["remove wire"] = document.createElement("li");
contextOptions["remove wire"].innerHTML =
  '<i class="material-icons">delete</i><span>Remove (Del)</span>';
contextOptions["remove wire"].onclick = () => {
  if (findWireByPos()) {
    action("disconnect", findWireByPos(), true);
  }
};
...
contextOptions["remove all"].innerHTML =
  '<i class="material-icons">delete</i><span>Remove (Del)</span>';
contextOptions["remove all"].onclick = () => {
  action("removeSelection", [...selecting.components], true);
};

// Input/Output
contextOptions["view connections"] = document.createElement("li");
contextOptions["view connections"].innerHTML =
  '<i class="material-icons">compare_arrows</i><span>View connections</span>';
...
contextOptions["componentize"].innerHTML =
  '<i class="material-icons">settings_input_component</i><span>Componentize</span>';
contextOptions["componentize"].onclick = () => {
  const component = new Custom(selecting.components, {
...
contextOptions["set waypoint"].innerHTML =
  '<i class="material-icons">my_location</i><span>Set waypoint (S)</span>';
...
contextOptions["goto waypoint"].onmouseenter = () => {
  waypointsMenu.show({
    ...
    y:
      -contextOptions["goto waypoint"].getBoundingClientRect().top / zoom +
      offset.y,
  });

  for (let i in contextOptions) {
    i != "goto waypoint" &&
      (contextOptions[i].onmouseover = function () {
        waypointsMenu.hide();
        for (let i in contextOptions)
          i != "goto waypoint" && (contextOptions[i].onmouseover = undefined);
      });
  }
};

function stringify_old(area = components) {
  ...
  for (let i of area) {
    let component = [i.constructor.name];
    ...
    if (i.output) {
      for (let output of i.output) {
        if (area.indexOf(output.to) == -1)
          area.splice(area.indexOf(output.to), 1);
      }
    }
...
function parse_old(string, dx, dy, select) {
  ...
  for (let i of string.components.reverse()) {
    ...
    if (component.constructor == Wire) {
      if (
        component.pos[0].x % 1 == 0 &&
        component.pos[0].y % 1 == 0 &&
        component.pos.slice(-1)[0].x % 1 == 0 &&
        component.pos.slice(-1)[0].y % 1 == 0
      ) {
    ...
    component.name =
      ...
      "#" +
      components.filter((n) => n.constructor == component.constructor).length;
    component.constructor == Wire

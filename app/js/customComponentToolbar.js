const customComponentToolbar = document.getElementById(
...
customComponentToolbar.querySelector(".close").onmouseup = function () {
  ...
  customComponentToolbar.querySelector("#name").innerHTML = path
    .slice(1)
    .map((a) => a.name)
    .join(" > ");

  customComponentToolbar.menu.style.opacity = 0;
  customComponentToolbar.menu.style.transform =
    "scale(.5) translateX(80px) translateY(-40px)";
  setTimeout(() => (customComponentToolbar.menu.style.display = "none"), 200);
  customComponentToolbar.querySelector(".edit").style.transform =
    "rotateZ(0deg)";
...
customComponentToolbar.show = function () {
  ...
  this.querySelector("#name").innerHTML = path
    .slice(1)
    .map((a) => a.name)
    .join(" > ");

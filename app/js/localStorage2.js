...
function getLocalStorage() {
  ...
  if (data.clipboard) {
    try {
      ...
      clipboard.copy(parsed.components, parsed.wires, parsed.selection);
    } catch (e) {
...
function stringify(components = [], wires = [], selection) {
  ...
  for (let i = 0; i < components.length; ++i) {
    ...
    if (constructor == "Custom") {
      data.componentData = JSON.parse(
        stringify(component.components, component.wires)
      );
    }
  ...
  for (let i = 0; i < wires.length; ++i) {
    ...
    const toPortIndex =
      wire.to && wire.to.component && wire.to.component.input.indexOf(wire.to);
...
function parse(data) {
  ...
  for (let i = 0; i < wires.length; ++i) {
    if (wires[i].length == 11) {
      ...
      const wire = new Wire(pos, intersections, color);
...
function saveBoard(name, components = window.components, wires = window.wires) {
...
function readFile(input) {
  ...
  reader.onload = function () {
    ...
    addSelection(clone.components, clone.wires);
    // } catch(e) {

...
dialog.welcome = function (component) {
  ...
  dialog.container.innerHTML +=
    "<i class='material-icons' style='font-size: 120px'>memory<i>";
  dialog.container.innerHTML +=
    "<p>Welcome to <span style='font-family: Righteous; font-size: 24px;'>BOOLR</span> !</p>";
  dialog.container.innerHTML +=
    "<p>If i'm right this is the first time you are using BOOLR.</p>";
  dialog.container.innerHTML +=
    "<p>Need a tutorial to learn how everything works?</p>";
...
dialog.createBoard = function () {
  ...
  dialog.container.innerHTML +=
    "<i class='material-icons' style='font-size: 120px'>save<i>";
  dialog.container.innerHTML +=
    "<p>This board doesn't have a save file yet. Want to create one?</p>";
...
dialog.editBoard = function (save) {
  ...
  dialog.addOption("OK", async () => {
    if (
      boardName.value != save.name &&
      boardName.value.length > 0 &&
      boardName.value.length < 100
    ) {
...
dialog.update = function (component) {
  ...
  dialog.container.innerHTML +=
    "<i class='material-icons' style='font-size: 60px'>update<i>";
...
dialog.connectToServer = function () {
  ...
  dialog.container.innerHTML +=
    "<i class='material-icons' style='font-size: 60px'>dns<i>";
...
dialog.connections = function (component) {
  ...
  dialog.container.innerHTML += `${
    component.name
  } has ${connections} connection${connections == 1 ? "" : "s"}. <br><br>`;

  if (input.children.length > 0) {
    dialog.container.innerHTML += `${input.children.length} connection${
      input.children.length == 1 ? "" : "s"
    } from:`;
    dialog.container.appendChild(input);
  }

  if (output.children.length > 0) {
    dialog.container.innerHTML += `${output.children.length} connection${
...
dialog.truthTable = function (type) {
  ...
  dialog.container.innerHTML +=
    "<span style='font-size: 50px; padding: 50px'>" +
    component.icon.text +
    "</span><br>";
  ...
  for (let i = 0; i < length; ++i) {
    const tr = document.createElement("tr");
    const input = ("0".repeat(component.input.length) + i.toString(2)).slice(
...
dialog.settings = function (component) {
  ...
  dialog.container.innerHTML +=
    "<i class='material-icons' style='font-size: 60px'>settings<i>";
...
dialog.localStorageError = function () {
  ...
  dialog.container.innerHTML +=
    "<i class='material-icons' style='font-size: 60px'>warning<i>";
  dialog.container.innerHTML +=
    "<p>Your browser doesn't allow this application to store data locally. " +
...
dialog.colorPicker = function (callback = (a) => a) {
  ...
  dialog.container.innerHTML +=
    "<i class='material-icons' style='font-size: 60px'>color_lens<i>";
...
dialog.editCustom = function (component) {
  ...
  dialog.container.appendChild(document.createTextNode("Width: "));
  ...
  dialog.container.appendChild(document.createTextNode("Height: "));
  ...
  dialog.addOption("OK", () => {
    ...
    if (description.value.length > 0)
      component.properties.description = description.value;
...
dialog.save = function (text) {
  ...
  dialog.container.innerHTML +=
    "<i class='material-icons' style='font-size: 60px'>save<i>";
  dialog.container.innerHTML +=
    "<p>This board will be saved as a .board file</p>";
  dialog.container.innerHTML += "Name: ";

  let input = document.createElement("input");
  input.setAttribute(
    "placeholder",
    "BOOLR-Save-" + new Date().toLocaleString()
  );
  dialog.container.appendChild(input);

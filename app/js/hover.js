...
hoverBalloon.show = function (component) {
  ...
  if (component.constructor == Wire) {
    ...
    this.innerHTML +=
      Math.round(component.pos.x) + "," + Math.round(component.pos.y) + "<br>";
    this.innerHTML += "ID: " + component.id + "<br>";
    this.innerHTML +=
      "Placed by: " +
      (component.placedBy ? component.placedBy : "you") +
      "<br>";
...
hoverBalloon.hide = function () {
  ...
  setTimeout(
    () => !hoverBalloon.display && (hoverBalloon.style.display = "none"),
    200
  );
};
...
for (let i = 0; i < document.getElementsByClassName("slot").length; ++i) {
  document.getElementsByClassName("slot")[i].onmouseover = function (e) {
    this.hover = true;
    ...
    if (toolbartip.style.display === "block") {
      ...
      if (i > 0 && i < 5) {
        toolbartip.innerHTML +=
          "<br><span style='font-size: 10px; color: #555'>Right click for details</span>";
      }
      toolbartip.style.top = `${this.getBoundingClientRect().top}px`;
      toolbartip.style.left = "60px";
    } else {
      ...
      setTimeout(() => {
        ...
        toolbartip.style.transform = "translateY(10px)";
      }, 1);
      toolbartip.style.top = `${this.getBoundingClientRect().top}px`;
      toolbartip.style.left = "60px";
      setTimeout(
        () =>
          (toolbartip.style.transition =
            "transform .2s, opacity .2s, left .2s"),
        100
      );
    }
  };
  ...
  document.getElementsByClassName("slot")[i].onmouseleave = function (e) {
    ...
    setTimeout(() => {
      ...
      for (let j = 0; j < document.getElementsByClassName("slot").length; ++j) {
        if (document.getElementsByClassName("slot")[j].hover)
          removeTooltip = false;
      }
...
for (let i = 0; i < document.querySelectorAll("#credits button").length; ++i) {
  document.querySelectorAll("#credits button")[i].onmouseenter = function (e) {
    ...
    if (creditstip.style.display == "block") {
      ...
      creditstip.style.left =
        this.getBoundingClientRect().left +
        this.clientWidth / 2 -
        creditstip.clientWidth / 2;
    } else {
      setTimeout(() => {
        if (this.hover) {
  ...
  document.querySelectorAll("#credits button")[i].onmouseleave = function (e) {
    ...
    setTimeout(() => {
      ...
      for (
        let j = 0;
        j < document.querySelectorAll("#credits button").length;
        ++j
      ) {
        if (document.querySelectorAll("#credits button")[j].hover)
          removeTooltip = false;
      }
